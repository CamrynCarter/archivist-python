"""Create an event for a publoc asset given url to Archivist and user Token.
...
to create a new asset for the archivist through archivist connection. The main function then
...
def create_event(arch, asset):
    ...
    return arch.publicevents.create(
        asset["identity"], props=props, attrs=attrs, confirm=True
    )
...
def create_asset(arch):
    ...
    #
    # This is a public asset so specify public as True
    return arch.assets.create(props={"public": True}, attrs=attrs, confirm=True)
...
def main():
    ...
    create an example archivist connection and create a public asset.
    The main function then uses the public asset to create an event for
    ...
    # Fetch the event - this is a public asset ro use the publicevents.py
    event = arch.publicevents.read(event["identity"])
    print("Event", event)
